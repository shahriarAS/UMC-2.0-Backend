...
const orderViewAll = async (req, res) => {
  try {
    const existorder = await orderModel
      .find()
      .populate("course")
      .populate("student");
    if (existorder) {
  ...
  try {
    ...
    if (existorder) {
      ...
      if (req.body.order_status == "approved") {
        const existCourse = await courseModel.findOne({
          _id: existorder.course,
        });
        await courseModel.updateMany(
          { _id: existorder.course },
          {
            $push: { enrolledStudent: existorder.student },
            $set: { enrolledCount: existCourse.enrolledCount + 1 },
          }
        );
        await userModel.updateOne(
          { _id: existorder.student, role: "student" },
          {
            $push: {
              enrolledCourse: existCourse._id,
              notifications: {
        ...
        await courseModel.updateMany(
          { _id: existorder.course },
          {
            $pull: { enrolledStudent: existorder.student },
            $set: { enrolledCount: existCourse.enrolledCount - 1 },
          }
        );
        await userModel.updateOne(
