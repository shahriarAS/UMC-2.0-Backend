...
const pdfView = async (req, res) => {
    try {
        ...
        if (existPDF) {
            await pdfModel.updateOne({ _id: req.params.pdfID }, { $set: { views: (existPDF.views + 1) } })
            res.status(200).json({
                result: existPDF,
...
const pdfCreate = async (req, res) => {
    try {
        ...
        await newPDF.save()
        await classModel.updateOne({ _id: newPDF.class }, { $push: { pdfs: newPDF._id } })
        await chapterModel.updateOne({ _id: newPDF.chapter }, { $push: { pdfs: newPDF._id } })
        await partModel.updateOne({ _id: newPDF.part }, { $push: { pdfs: newPDF._id } })
        await courseModel.updateOne({ _id: newPDF.course }, { $push: { pdfs: newPDF._id } })
        res.status(200).json({
...
const pdfUpdate = async (req, res) => {
    try {
        ...
        if (existPDF) {
            await pdfModel.updateOne({ _id: req.params.pdfID }, { $set: { ...req.body, updated_at: Date.now() } })
            res.status(200).json({
...
// Export
export { pdfView, pdfViewEnrolled, pdfCreate, pdfUpdate, pdfDelete }